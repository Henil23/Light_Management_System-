...
def register_user(user_input_module, user_auth_module):
    ...
    user_auth_module.register_user(first_name, last_name, employee_number, email, date_of_birth, contact, address, username, password)

def authenticate_user(user_input_module, user_auth_module):
    auth_username = user_input_module.get_user_input("Enter username for authentication: ")
    auth_password = user_input_module.get_password_input()
    user_auth_module.authenticate_user(auth_username, auth_password)
